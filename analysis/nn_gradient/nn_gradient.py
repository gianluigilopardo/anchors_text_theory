...
TRAIN = True
PATH = os.path.join(os.getcwd(), 'nn_gradient', vect, f'net_{DATASET}')

if TRAIN:
    # CLASSIFIER
    model = nn.Sequential(nn.Linear(x_train.shape[1], 64),
                          nn.ReLU(),
                          nn.Linear(64, df.stars.nunique()),
                          nn.Softmax(dim=1))
    ...
    model = nn.Sequential(nn.Linear(x_train.shape[1], 64),
                          nn.ReLU(),
                          nn.Linear(64, df.Review.nunique()),
                          nn.Softmax(dim=1))
...
info = {'network': 'model = nn.Sequential(nn.Linear(x_train.shape[1], 64), nn.ReLU(), '
                   'nn.Linear(64, df.Review.nunique()), nn.Softmax(dim=1))',
        'loss': 'nn.NLLLoss()',
...
res = os.path.join(os.getcwd(), 'nn_gradient', vect)
