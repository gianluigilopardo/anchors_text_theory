"""
Training a 3 layers neural network with Restaurants dataset
"""
...
# CLASSIFIER
model = nn.Sequential(nn.Linear(x_train.shape[1], 500),
                      nn.ReLU(),
...
optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=0.1)  # 0.840
# optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=0.1)  # 0.816
...
best_accuracy = 0
best_model = []
PATH = os.path.join(os.getcwd(), f'nn_3_{DATASET}.p')
