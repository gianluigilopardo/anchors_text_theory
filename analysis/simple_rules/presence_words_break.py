...
# Example to explain
# pick any document with positive prediction
example = "Food is very very very very very good!"
print(example)
anchors_res = []
...
info = {'Description': 'Classifier returning 1 if \'good\' and \'very\' are present\n'
                       'Break case: the multiplicity of \'very\' in the example cross the breakpoint value B=4',
        'Example': example, 'N_runs': N_runs,
        'Multiplicities': multiplicities,
        'Anchors': anchors_res}
path = os.getcwd().replace('analysis', 'results')
pickle.dump(info, open(os.path.join(path, str('presence_words_break.p')), 'wb'))
